...
One of that largest online retailers, Amazon, has managed to change the worldwide behavior and meet consumer expectations by ensuring everything is connected. Therefore, everything is accessible by the click of a button. Amazon, has continued to decrease delivery times from weeks to now days. They have even gone further enough to shorten the delivery time of certain goods to "Next Day" and most recently the possibility for a "1 Hour" delivery. They have made this possible by building smart distribution centers in various locations and implementing a highly intelligent delivery process. Without a sound and sophisticated delivery infrastructure, this worldwide success would not be possible.
...
2.)	Improved battery life and speed will significantly reduce the overall number of drones needed by at least 20%. Therefore, reducing the logistical footprint and costs for the respective logistical company - making it easier, cheaper and more probable to implement. 
...
To do so, we focused our attention on demonstrating how a drone delivery system would be implemented. This approach enables us to determine which scale of effort is needed and also define the quality and quantity of drones it would take to provide an efficient air-based delivery system for the intended area. Changing certain parameters on the drone such as battery density, endurance, max speed, charging time, etc. can all help to predict completion estimates for newer technology.

To successfully model a simulation that would answer the fundamental questions above, we had to define certain prerequisites and assumptions beforehand. We defined these prerequisites and assumptions based on the drone market screening we gathered. To meet our expectations and improve the overall validity of the system we chose to use a drone created by DJI company. This company is one of the world leading civil drone manufacturers and, with the chosen drone, offers the best compromise between carrying power, in other words how much weight it can transport and lift, and endurance. Endurance is an important element within our simulation as it helps define the reach of the drone and overall radius.
...
Below is the simulated design process for the AnyLogic Model. For the purpose of the first simulation runs of this model, the simulation starts with 5 / 8 / 11 drones for a runtime of 600 minutes (10 hours). Within this runtime, there is only 1 order per household (HH) per hour (13 orders/hour) for the area. Here, we aim to determine how drone utilization and optimization varies based on package amount.
...
- "fast": the speed will be adapted to 60 mph (88ft/sec, 29m/sec). Some modern drones can achieve that speed and are foreseen to be faster, depending on the type of drones. 
...
All other values (houses, number of drones, ...) will remain the same as the first half of the simulation to be able to compare the impact of the attributes Speed and Charge.
...
The team decided to additionally simulate this process as discrete system. We employed the techniques of a Monte-Carlo Simulation. This model was made to give another perspective to the problem using different framework and a slightly different model. 
...
This function starts off by creating the necessary constraints - using Time equal to distance over Velocity. Using this newly calculated array we can simulate each delivery by taking into consideration the flight time with respect to the battery life and capacity. This allows us to monitor the battery life. To start the model was developed asking if drone 1's batteries are charged, if not drone 2, and then drone 3. If battery life was at half capacity or lower, the next in line would be sent out, and the rest were charging. Various statistics are then accounted for and collected. A Monte-Carlo Simulation is set-up and ran for a number amount of trials to further provide a variety number of statistics.
...
After running the simulation, statistics revealed different values and statistics for the different numbers of order/HH/hour, with 1 order/HH/hour with an optimum 11 drones having a utilization rate = 81%, fulfilled deliveries = 94, and a max delivery time < 40 minutes. 2 orders/HH/hour with an optimum of 18 drones have a utilization rate = 85%, fulfilled deliveries = 164 and a max delivery time < 30 minutes. 25 drones for 3 orders/HH/Hour show a utilization rate = 85%, fulfilled deliveries = 217 and a max delivery time  < 28 minutes.
...
Figure 2 shows the effects on 1 order/HH/hour for the given area and 11 drones. The optimization process shows slight variations in necessary drones:  
  ...
  - 1_11both 	optimal number of drones: 9      --> that's a change of 19% from normal to both
...
Figure 3 shows the effects on the 2 order/HH/hour for the given area and 18 drones. The optimization process shows again positive variations in necessary drones (decrease of up to 3, which equals a 17% reduction):  
  ...
  - 2_18both 	optimal number of drones: 15     --> that's a change of 17% from normal to both
...
Lastly, Figure 4 shows the effects on 3 order/HH/hour and 25 drones. The optimization process shows again positive variations in necessary drones (decrease of up to 7, which equals a 28% reduction) for a targeted utilization rate of 85%:  
  ...
  - 3_25both 	optimal number of drones: 18     --> that's a change of 28% from normal to both!
...
Based upon data simulated above, we could say that a unit of drones with in the specified range and battery life could deliver 200 packages efficiently. We can see this from the frequency graphs depicted from the results section. The total simulated time for the run with 200 orders was around 26-27hrs. For 100 orders the total time for all deliveries was around 12.5 hrs. This model shows the importance of multi-drone units and understanding the usage frequency in order to make maintenance repairs. For example if a unit is in the middle of the 12.5 hour shift of 100 orders to be delivered, suddenly needs a repair. With this model it can be seen that if one drone does need maintenance it most likely wont be an issue for possibly 10-20 deliveries. This is huge because then one drone could then be fixed to its specifications (within a certain time frame), while the other drones are in use, and then put back into queue waiting to be deliver again. Although to deliver 100 packages under an hour we will need more than 1 drone and this is shown in the python simulation. 
